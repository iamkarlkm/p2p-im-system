        ...
        if learning_log_path.exists():
            try:
                ...
                for line in recent_lines:
                    if line.strip() and ('学习' in line or '了解' in line):
                        new_points.append(line.strip()[:60])
            except:
                pass
...
"""
        if roadmap_info['new_features']:
            report += f"- 新增功能点：{len(roadmap_info['new_features'])}个\n"
            for feature in roadmap_info['new_features']:
                report += f"  - {feature}\n"
    else:
...
"""
        if knowledge_info['new_points']:
            report += f"- 新知识点：{len(knowledge_info['new_points'])}个\n"
            for point in knowledge_info['new_points']:
                report += f"  - {point}\n"
    else:
    ...
    if plan_info['exists'] and 'features' in plan_info:
        report += "| 功能名称 | 状态 |\n|----------|------|\n"
        for feature in plan_info['features'][:12]:  # 只显示前12个
            name = feature['name'][:40] if len(feature['name']) > 40 else feature['name']
            report += f"| {name} | {feature['status']} |\n"
...
- 待开发：{counts['pending']}个
  - 待人工解决：{counts['manual_resolution']}个
"""
    else:
...
def save_report(report):
    ...
    with open(log_file, 'w', encoding='utf-8') as f:
        ...
        recent_entries = entries[-8:] if len(entries) > 8 else entries
...
if __name__ == "__main__":
    try:
        ...
        # 输出纯文本报告（不含特殊字符）
        plain_report = report.replace('✅', '').replace('🔄', '').replace('📋', '').replace('⚠️', '')
        print(plain_report)
