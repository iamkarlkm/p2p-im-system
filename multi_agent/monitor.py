...
def check_roadmap(roadmap_path):
    ...
    try:
        ...
        # 简单分析roadmap内容
        planning_count = content.count('## 规划中') + content.count('## 计划中')
        new_features = []
            ...
            if kf['ctime'] > cutoff_time:
                ...
                try:
                    with open(kf['path'], 'r', encoding='utf-8') as f:
                        ...
                        # 提取前几行作为知识点
                        lines = content.split('\n')[:5]
                        new_knowledge.append(f"{kf['name']}: {' '.join(lines).strip()[:100]}...")
                except:
    ...
    try:
        ...
        for line in lines:
            ...
            if line.startswith('|') and '|' in line[1:]:
                ...
                if len(parts) >= 3 and '功能名称' not in line and '---' not in line:
    ...
    if os.path.exists(log_path):
        try:
            with open(log_path, 'r', encoding='utf-8') as f:
                ...
                # 尝试提取上一次的代码行数
                import re
                match = re.search(r'总代码行数：\[(\d+)\]', previous_content)
                if match:
...
- 功能模块变化：共{len(dev_plan_info['features'])}个功能
...
"""
    ...
    return {
        ...
        'features_total': len(dev_plan_info['features']),
...
if __name__ == "__main__":
    ...
    print(f"- 功能总数: {result['features_total']}个 (已完成{result['completed_features']}个)")
